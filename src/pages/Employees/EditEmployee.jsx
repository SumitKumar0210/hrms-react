...
const validationSchema = Yup.object({
    ...
    // On edit the confirm field is optional when the user hasn't changed the account number.
    // We use a custom test: if the user has typed anything in confirmAccountNumber it must
    // match accountNumber; if they've left it blank we skip the check (account unchanged).
    confirmAccountNumber: Yup.string().test(
        "confirm-match",
        "Account numbers do not match",
        function (value) {
            const { accountNumber } = this.parent;
            // blank = user didn't re-enter (no change) → valid
...
const EditEmployee = () => {
    ...
    // Populate existing documents map
    useEffect(() => {
    ...
    // Sync selected shift when data loads
    useEffect(() => {
    ...
    const handleSubmit = async (values, { setSubmitting, setErrors }) => {
        try {
            ...
            // Only send confirm when the user re-entered it (account changed)
            ...
            // formData.append("bank_aadhar_no", values.bankAadharNo.trim());
    ...
    // Bank detail helpers — pull from nested bankDetail relation if it exists,
    // otherwise fall back to top-level fields depending on your API shape.
    ...
    const initialValues = {
        ...
        // Documents (always null — new uploads only)
        idProof: null,
        addressProof: null,
        bankDetails: null,
        contractLetter: null,
        ...
        // Bank details — pre-populated from API
        ...
        confirmAccountNumber: "", // intentionally blank — user must re-enter only if changing
        panNumber: bankDetail.pan_number || currentEmployee.pan_number || "",
        // bankAadharNo: bankDetail.aadhar_number || currentEmployee.bank_aadhar_no || "",
                        ...
                        <Form onSubmit={handleSubmit}>
                            <Card className="mt-3 p-3 shadow-sm border-0">
                                <Card.Body>
                                    ...
                                    <Row className="px-2">
                                            ...
                                            <Form.Group>
                                                ...
                                                <div className="input-group">
                                                    <Form.Control
                                                        ...
                                                        onChange={(e) =>
                                                            setFieldValue("accountNumber", e.target.value.replace(/\D/g, ""))
                                                        }
                                                        onBlur={handleBlur}
                                        ...
                                        <Col md={3} className="mb-3">
                                            <Form.Group>
                                                ...
                                                <Form.Text className="text-muted">
                                                    Paste disabled — leave blank to keep existing account
                                                </Form.Text>
                                            </Form.Group>
                                        </Col>
                                    ...
                                    {/* Row 3: PAN + Aadhaar + Is Micro + counters */}
                                    <Row className="px-2">
                                        <Col md={3} className="mb-3">
                                            <Form.Group>
                                                <Form.Label>
